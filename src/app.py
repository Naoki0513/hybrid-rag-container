...
QUERY_GENERATOR_MODEL_ID = os.environ["QUERY_GENERATOR_MODEL_ID"]
PARENT_CHUNK_SIZE = int(os.environ["PARENT_CHUNK_SIZE"])
CHILD_CHUNK_SIZE = int(os.environ["CHILD_CHUNK_SIZE"])
RETRIEVER_SEARCH_K = int(os.environ["RETRIEVER_SEARCH_K"])
...
def generate_word_ngrams(text: str, i: int, j: int, binary: bool = False) -> List[tuple]:
   ...
   Args:
       ...
       i (int): n-gramの最小文字数
       j (int): n-gramの最大文字数
       binary (bool, optional): Trueの場合、重複を削除. Defaults to False.
       
   Returns:
       List[tuple]: n-gramのリスト
   """
   try:
...
# 説明: 上記の関数は、Sudachiを用いて文書をトークナイズし、n-gramを生成する機能を担っています。
# これにより文書を適切に分割し、キーワード検索に利用できる形に変換します。 

def preprocess_func(text: str) -> List[str]:
   return generate_word_ngrams(text, 1, 1, True)

def create_retriever(texts: List[str]) -> BM25Retriever:
   return BM25Retriever.from_texts(texts, preprocess_func=preprocess_func, k=RETRIEVER_K)

# 説明: 上記の関数は、BM25検索器を作成します。事前に分割・トークナイズされた文書を受け取り、
# 検索に必要なデータ構造を構築します。これにより、ユーザーの質問に関連する文書をキーワードベースで効率的に検索できるようになります。
...
# 説明: 上記の関数は、質問文からキーワードを抽出するためのLLMチェーンを作成します。
# LLMにプロンプトを渡して実行することで、質問文から重要なキーワードを抽出します。
# これらのキーワードはBM25検索器に渡され、関連ドキュメントの検索に利用されます。
...
# 上記の関数は、BM25で絞り込まれたドキュメントに対してベクトル化を行い、FAISSベクトルストアに格納します。
# これにより、質問文とドキュメントの意味的な類似度に基づいて再ランク付けを行うことができます。
# ParentDocumentRetrieverを使用することで、ドキュメントを再帰的に分割し、より細かい粒度で検索を行います。 
...
# 上記の関数は、ベクトル検索で得られた関連ドキュメントとユーザーの質問文を組み合わせてLLMにプロンプトを送信し、
# 最終的な回答を生成します。ここでは、Claude 3 Sonnetモデルを使用して自然な日本語の回答文を生成します。
...
def process_question(question: str) -> str:
   ...
   print(rephrase_retriever.llm_chain.run(question))
...
# 上記の関数は、一連の処理を統合し、ユーザーの質問に対する最終的な回答を生成します。
# 1. JSONファイルからドキュメントデータを読み込みます。
# 2. BM25検索器を作成し、関連ドキュメントを検索します。
# 3. LLMを使用して質問文からキーワードを抽出し、検索クエリを生成します。 
# 4. 関連ドキュメントをベクトル化し、FAISSベクトルストアに格納します。
# 5. ベクトル検索を行い、最終的な関連ドキュメントを取得します。
# 6. 関連ドキュメントとユーザーの質問文を組み合わせてLLMにプロンプトを送信し、最終的な回答を生成します。
